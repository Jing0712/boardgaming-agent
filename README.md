...
- 纯前端实现，无后端、无构建步骤，单文件 HTML 承载全部逻辑与样式
- 通过 OpenAI 兼容格式调用 Volcengine Ark 的 `/chat/completions` 接口
- 多人房间基于 Firebase Firestore 的实时监听（`onSnapshot`）实现跨设备同步，房主设备做唯一的 AI 调用中转
- 角色形象是内联 SVG 绘制的像素猫，跟着对话状态（待机 / 思考 / 说话 / 出错）切换表情，无需额外图片资源
